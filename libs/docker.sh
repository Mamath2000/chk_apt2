...
docker::stack_name_from_file() {
  local compose_file="$1"
  printf 'Docker %s' "${compose_file%/docker-compose.yml}"
}
...
docker::deployed_service_images_json() {
  local compose_file="$1"
  local services_json result service container_id image_id repo_digests digest

  if ! services_json="$(docker::service_images_json "$compose_file" false)"; then
    return 1
  fi
  ...
  while IFS= read -r service; do
    ...
    repo_digests="$(docker image inspect --format '{{json .RepoDigests}}' "$image_id" 2>/dev/null || printf '[]')"
