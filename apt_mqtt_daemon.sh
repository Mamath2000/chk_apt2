...
CLIENT_ID="$(tools::sanitize_hostname "${HOSTNAME}_apt_mqtt")"
...
tools::check_requirements mosquitto_pub mosquitto_sub jq apt-get apt git systemctl
...
tools::log DEBUG "Configuration active: object_id=$OBJECT_ID check_interval=$CHECK_INTERVAL mqtt_user_configured=$([ -n "$USERNAME" ] && printf true || printf false)"
...
publish_status() {
  ...
  tools::log DEBUG "Statut APT publié: packages=$count installed_version=$installed_version latest_version=$latest_version in_progress=$in_progress"
...
handle_install() {
  ...
  trap 'state::clear_in_progress || true' EXIT
  publish_status
  tools::log DEBUG "Début mise à jour APT: dry_run=$dry autoremove=$AUTOREMOVE"
  ...
  tools::log DEBUG "Fin mise à jour APT: rc=$rc next_packages=$(printf '%s' "$packages" | jq 'length') last_run=$last_run"
...
handle_self_update() {
  ...
  trap 'state::clear_in_progress || true' EXIT
  publish_status
  tools::log DEBUG "Début self-update: install_dir=$INSTALL_DIR service=$SERVICE_NAME"
  ...
  script_version="$(version::read)"
  tools::log DEBUG "Fin self-update: rc=$rc script_version=$script_version"
  ...
  if git::service_exists "$DOCKER_SERVICE_NAME"; then
    echo "Redémarrage du service $DOCKER_SERVICE_NAME..."
    git::restart_service "$DOCKER_SERVICE_NAME"
  fi
...
start_mqtt_subscription() {
  ...
  while true; do
    tools::log DEBUG "Souscription MQTT démarrée: topics=$CMD_TOPIC $GLOBAL_UPDATE_TOPIC"
    if mqtt::sub_many "$CMD_TOPIC" "$GLOBAL_UPDATE_TOPIC" > "$CMD_FIFO"; then
      rc=0
    else
      rc=$?
    fi

    tools::log WARN "Souscription MQTT interrompue: rc=$rc reconnexion dans 1s"
    sleep 1
  done
}
...
main() {
  ...
  (
    while true; do
      if read -r payload <&3; then
        ...
        tools::log DEBUG "Commande MQTT reçue: raw=$payload normalized=$payload_lc"
        case "$payload_lc" in
          install|update|upgrade|upgrade-all)
            start_background_job "apt-install" handle_install false
            ;;
          dry-run|simulate)
            start_background_job "apt-dry-run" handle_install true
            ;;
  ...
  while true; do
    ...
    tools::log DEBUG "Cycle suivant après tempo ($CHECK_INTERVAL s)"
  done
}
